      ...
      <b-form>
        <b-row>
          <b-col cols="7">
            <b-row class="m-1">
              <b-col cols="2">
                نام:<b-badge variant="success">{{ firstName }}</b-badge>
              </b-col>
              <b-col cols="3">
                نام خانوادگی:<b-badge variant="success">{{ lastName }}</b-badge>
              </b-col>
              <b-col cols="4">
                <span class="p-1">
              موجودی کل: <b-badge variant="success">{{ convertToCurrency(totalSum) }}ریال </b-badge>
                </span>
              </b-col>
              <b-col cols="3">
                تعداد قرارداد:<b-badge variant="success">{{ totalCount }}</b-badge>
              </b-col>
            </b-row>
            <b-row class="m-1">
              <b-col cols="5">
          ...
          <b-col cols="12">
            <b-button
                variant="success"
                type="submit"
                @click.prevent="stuffResponse('to_boss_confirm')"
                class="m-1"
            >
              تایید
            </b-button>
...
import {
  TOTAL_COUNT_GHARARDAD_AND_MOJOODI,
  TRANSACTION_BOSS_CONFIRM,
  TRANSACTION_CONFIRM_WITH_STUFF,
  TRANSACTION_STUFF_CONFIRM
    ...
    this.$apollo.mutate({
      ...
      variables: {
        ...
        .then((result) => {
          ...
          this.$apollo.mutate({
            mutation:TOTAL_COUNT_GHARARDAD_AND_MOJOODI,
            variables:{
              id:item.profile.id
            }
          }).then((result)=>{
            this.totalSum=result.data.transactions.totalSum
            this.totalCount=result.data.transactions.totalCount
          })
          ...
          this.firstName=item.profile.firstName
          this.lastName=item.profile.lastName
